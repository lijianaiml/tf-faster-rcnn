...
class DataAugmentForObjectDetection():
  ...
  def _rotate_img_bbox(self, img, bboxes, angle=5, scale=1.):
    ...
    # 仿射变换
    rot_img = cv2.warpAffine(img, rot_mat, (int(
        math.ceil(nw)), int(math.ceil(nh))), flags=cv2.INTER_LANCZOS4)
  ...
  def _crop_img_bboxes(self, img, bboxes):
    ...
    crop_bboxes = list()
    for bbox in bboxes:
      crop_bboxes.append([bbox[0] - crop_x_min, bbox[1] -
                          crop_y_min, bbox[2] - crop_x_min, bbox[3] - crop_y_min])
...
source_pic_root_path = '/data/face_data/20181128/JPEGImages'
source_xml_root_path = '/data/face_data/20181128/Annotations'
...
#           cnt += 1
    ...
    # print(coords_aug)
    # 保存增强图片并生产xml文件
    img_name = '0%s.jpg' % (file[:-4])
    img_size = auged_img.shape

    save_aug_img(auged_img, "./output_mirror_img/%s" % img_name)
    generate_xml(img_name, coords_aug,
                 img_size, "./output_mirror_xml")
